# -*- coding: utf-8 -*-
"""
Created on Wed Oct 31 14:12:27 2018
...
class Cell(nn.Module):#if reduction=true- then cell will be reduction
  ...
  def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev):
    ...
    for i in range(self._steps):#nodes in cell- the get info from j
      for j in range(2):#for j in range(2+i):#change
  ...
  def forward(self, s0, s1, weights):#cell gets as input the previos outputs. weights is arch_weights:
    ...
    states = [s0, s1]
    states_initial = states.copy()
    offset = 0
    for i in range(self._steps):#=0
        #s = sum(self._ops[offset+j](h, weights[offset+j]) for j, h in enumerate(states))#change
        s = sum(self._ops[offset+j](h, weights[offset+j]) for j, h in enumerate(states_initial))#here we get the mixed_op value on feature_maps 0 and 1 
        offset += len(states_initial)#change
        #offset += len(states)
        states.append(s)
...
class Network(nn.Module):
  ...
  def _initialize_alphas(self):
    k = sum(1 for i in range(self._steps) for n in range(2))#change
  ...
  def genotype(self):
    ...
    def _parse(weights):
      ...
      for i in range(self._steps):
        ...
        #edges = sorted(range(i + 2), key=lambda x: -max(W[x][k] for k in range(len(W[x])) if k != PRIMITIVES.index('none')))[:2]#change
        edges = sorted(range(2), key=lambda x: -max(W[x][k] for k in range(len(W[x])) if k != PRIMITIVES.index('none')))[:2]
        for j in edges:
          k_best = None
